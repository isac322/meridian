...
- **Nested Anthropic server tools** — Meridian is not currently a transparent Anthropic API passthrough for Messages requests that use server-executed tools such as `web_search_20250305` / `web_search`. This is easy to hit from OpenCode plugins like [`opencode-websearch`](https://github.com/emilsvennesson/opencode-websearch): OpenCode first calls the plugin's client-side `web-search` tool, then the plugin sends a second Anthropic Messages request that expects native server-tool response blocks such as `server_tool_use` and `web_search_tool_result`. If that nested Anthropic provider also points at Meridian, the request goes through Meridian's Claude Agent SDK translation path instead of native Anthropic server-tool passthrough semantics. Configure such plugins to use a non-Meridian search provider, or track #488 for first-class support.
